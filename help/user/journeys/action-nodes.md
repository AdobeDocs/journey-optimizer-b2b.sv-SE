...
Under din resa kan du lägga till en _[!UICONTROL Take an action]_-nod för att utföra en åtgärd, som att skicka ett e-postmeddelande, ändra ett poängtal, tilldela till en inköpsgrupp och så vidare. Åtgärder är vanligtvis vad du vill ska hända som ett resultat av någon typ av utlösare, till exempel en händelse eller en tidigare åtgärd.

![Video](../../assets/do-not-localize/icon-video.svg){width="30"} [Se översiktsvideon](#overview-video)

## Kontoåtgärder

Använd en kontoåtgärd när du vill tillämpa en ändring på alla personer som ingår i konton på nodsökvägen under en kontoresa.
...
Använd en åtgärd för personer på ett konto eller en personresa när du vill tillämpa en ändring för alla personer på nodsökvägen. För en kontoresa kan den här nodtypen användas inom den _delade sökvägen av personer_ eller _dela upp sökvägen efter konton_.

### Åtgärder och begränsningar {#people-action-constraints}

| Kontext | Åtgärd | Resetyp | Begränsningar |
| ------- | ------ | ------------ | ----------- |
| [Journey Optimizer B2B](#journey-optimizer-b2b-actions) | [!UICONTROL Add to external customer audience] | <li>Kontoresa <li>Personresa | <li>Välj extern kundpublik |
| | [!UICONTROL Assign to Buying Group] | <li>Kontoresa | <li>Välj lösningsintresse <li>Välj roll |
| | [!UICONTROL Change Score] | <li>Kontoresa | <li>Efternamn <li>Förändring i poäng |
...
| | [!UICONTROL Send WhatsApp] | <li>Kontoresa | <li>Skapa whatsApp-meddelande |
...
| | [!UICONTROL Add to Marketo list] | <li>Kontoresa <li>Personresa | <li>Namn på extern Marketo-anslutning <li>Listnamn |
| | [!UICONTROL Remove from Marketo list] | <li>Kontoresa <li>Personresa | <li>Namn på extern Marketo-anslutning <li>Listnamn |
...
Använd den här åtgärden för att [skapa personaliserade upplevelser](../content/web-experiences.md) direkt på din webbplats. Webbkanalsfunktionen erbjuder en flexibel verktygslåda som du kan använda för att förbättra engagemanget med skräddarsytt webbinnehåll.

![Utför en åtgärd - Anpassa webbupplevelsen](./assets/node-action-person-personalize-web-experience.png){width="300"}
...
>[!NOTE]
>
>Du kan använda borttagning av dubbletter via e-post i kontoresor för att se till att samma e-post inte skickas flera gånger till samma e-postadress under en resa. Mer information finns i [E-postborttagning](../content/email-deduplication.md).
...
Använd den här åtgärden om du vill skicka ett WhatsApp-meddelande. Du kan skapa, anpassa och förhandsgranska whatsApp-meddelanden i den visuella designrymden (se [WhatApp authoring](../content/whatsapp-authoring.md)).

![Vidta en åtgärd - Skicka whatsApp](./assets/node-action-send-whatsapp.png){width="300"}
...
>[!NOTE]
>
>Marketo Engage-åtgärderna kräver konfigurerad integrering med en eller flera externa Marketo Engage-instanser. Mer information om den här konfigurationen finns i [_Aktivera Marketo Engage-anslutningar för att stödja åtgärder_](../admin/marketo-actions-connect.md).

Du kanske vill inaktivera kampanjer i Marketo Engage för personer som ingår i inköpsgrupper i Journey Optimizer B2B Edition. I det här fallet kan du skapa en statisk lista i Marketo Engage specifikt för att tillgodose lösningsbehovet. Använd sedan åtgärden _Lägg till i Marketo-lista_ från en kundnod på en delad sökväg genom att köpa en grupp. Den här åtgärden lägger till köpgruppsmedlemmar i en viss statisk lista i en ansluten Marketo Engage-instans. Använd sedan den statiska listan med lösningsintresse för ett smart listfilter i Marketo Engage.
