...
Om du vill lägga till villkorligt innehåll i e-postmeddelanden kan du använda villkorsregler som lagras i biblioteket _Villkor_ i Adobe Journey Optimizer. Använd villkorsregler i e-postdesignern när du [skapar ett e-postmeddelande inom en kontoresa](./email-authoring.md).
...
När du skapar ett e-postmeddelande för din kontoresa i e-postdesignern använder du villkorliga regler för att definiera flera varianter för en innehållskomponent.
...
1. Håll pekaren över den första varianten (_Variant - 1_) och klicka på ikonen _Markera villkor_ ( ![Villkorsikon](../assets/do-not-localize/icon-select-condition.svg) ).
   ...
   Om villkoret som du behöver inte finns [skapar du en villkorsregel](#create-a-conditional-rule) genom att klicka på **[!UICONTROL Create new]**.
...
Villkorliga regler är en uppsättning villkorsuttryck som kan utvärderas som sant eller falskt. Du kan använda dessa regler för att avgöra vilken innehållsvariant som ska visas i ett e-postmeddelande baserat på olika villkor, till exempel profilattribut eller sammanhangsbaserade händelser.
...
Du kan komma åt villkorsstyrd regelbyggaren från e-postdesignern när du väljer ett villkor för en komponentvariant.
