...
| ---- | ---- | ----------- |
| Funktion | Landningssidor | Nu kan marknadsförare skapa och publicera landningssidor i Journey Optimizer B2B edition för att stödja resor och program _(tidigare en Beta-programfunktion)_. |
| Funktion | Forms | Marknadsförarna kan nu skapa och publicera återanvändbara formulärkomponenter för att kunna skicka data från landningssidor som har skapats och publicerats i Journey Optimizer B2B edition _(tidigare en Beta-programfunktion)_. |
| Funktion | WhatsApp-kanal | Nu kan marknadsförare skicka whatsApp-meddelanden direkt från kontoresor via Meta Cloud API. Den här funktionen möjliggör smidig integrering av WhatsApp-meddelanden för att stödja kanaler för reseinnehåll. |
...
| Funktion | Anpassade externa åtgärder för resor | [!BADGE Förenklad arkitektur]{type=Informative tooltip="Finns med förenklad arkitektur"} Utvecklare kan nu använda API:er för att skapa integreringar med sina egna system. Med dessa anpassade integreringar kan marknadsförare lägga till noder för _extern åtgärd_ och _extern delad sökväg_ för att göra utgående begäranden till externa tjänster under kontoresan. |
| Funktion | Varumärken | (Beta) Marknadsföringsteamen kan upprätthålla ett enhetligt varumärke i sitt e-postinnehåll genom att lagra och hantera varumärkesprofiler. Genom att lägga till resurser som färger, teckensnitt, logotyper, teman, visuellt innehåll och riktlinjer för efterlevnad kan de använda varumärkesprofilen för att skapa generativt AI-innehåll. De kan också mäta varumärkesjusteringen för att säkerställa regelefterlevnaden. |
| Förbättring | Skicka e-post - optimering vid sändning | För _Skicka e-post_-åtgärdsnoder på personliga resor kan du använda alternativen för _optimering vid sändning_ för att anpassa tidsåtgången för e-postleverans genom att förutsäga när det är mest troligt att varje profil interagerar. |
| Förbättring | Verktyg för e-postdesign - expertläge | Med _expertläge_ i e-postdesignområdet kan användare göra mindre HTML/CSS-redigeringar och lägga till script-taggar i ett e-postmeddelande för att lösa återgivningsproblem. |
| Förbättring | Personanpassade objekt - köpa grupprollmallar | [!BADGE Förenklad arkitektur]{type=Informative tooltip="Finns med förenklad arkitektur"} När administratörer konfigurerar anpassade objekt som är relaterade till en företagsprofil kan marknadsförare nu definiera inköpsgrupproller med dessa anpassade objekt. |
...
| Typ | Objekt | Beskrivning |
| ---- | ---- | ----------- |
| Funktion | Samarbete med e-postinnehåll | Marknadsföringsteamen kan nu kommentera och samarbeta med andra Journey Optimizer B2B edition-användare i samband med en e-postresurs. De kan tagga teammedlemmarna så att de får ett e-postmeddelande med information om kommentaren. Meddelande finns också tillgängligt som ett pulsmeddelande. [Läs mer](../content/email-collaboration-tools.md) |
...
| Funktion | Personinformationssida | Användarna kan nu klicka på en persons namn när det visas (som en hyperlänk) på den intelligenta kontrollpanelen, sidan med information om köpgrupper och sidan med kontoinformation. Den här åtgärden öppnar sidan med information om den associerade personen, med information om kontakten, deras aktivitet och de mest engagerade inköpsgrupperna. [Läs mer](../accounts/person-details.md) |
| Funktion | Åtgärder för konto och inköpsgrupp | Vidta åtgärder direkt från kontoinformation och köp av informationssidor för grupper för snabbt och avsiktligt engagemang. <li>Använd åtgärden _Skicka e-post_ för att skicka ett godkänt Marketo Engage-e-postmeddelande till valda kontokontakter eller till medlemmar i inköpsgrupper. [Läs mer](../accounts/account-details.md#send-email) <li>Bland köpgruppsinformationen ingår även _Tilldela en ny medlem_, _Ta bort en medlem_ och _Redigera en roll_. [Läs mer](../buying-groups/buying-group-details.md#members-tab) |
...
| Förbättring | Fyra extra exempel på e-postmallar | Biblioteket med exempelmallar för e-post innehåller nu fyra _SecurFinancial_ -mallar som exempel för återengagemang, information, näring och feedback. |
...
| Funktion | Information om inköpsgrupp och konto | Se insikter på köpgrupps- och kontonivå för att få mer kontext och historiska data när ni börjar engagera en kund.<p>Köpgruppens information innehåller alla tredjepartsmetoder som upptäcks. <a href="../buying-groups/buying-group-details.md">Läs mer</a><p>Sidan med kontoinformation belyser den ökning av engagemang som upptäckts, så att marknadsförarna kan varna försäljningen om konton som är klara för anpassat säljfokuserat engagemang.  <a href="../accounts/account-details.md">Läs mer</a> |
...
| Funktion | _Lägg till i listan_ och _Ta bort från listan_ personåtgärder i resor | Använd reseåtgärder för att lägga till eller ta bort personer från en statisk lista i en sammankopplad Marketo Engage-instans som samordnas med er kontofärdssamordning. <a href="../journeys/action-nodes.md">Läs mer</a> |
