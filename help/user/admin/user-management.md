---
title: Användaråtkomst och behörigheter
description: Hantera användaråtkomst med Experience Cloud Admin Console - skapa användargrupper, tilldela produktprofiler och konfigurera rollbaserade behörigheter för Journey Optimizer B2B edition.
...
1. Som systemadministratör bör du få flera e-postmeddelanden från Adobe som en del av introduktionsprocessen.
   ...
   Leta upp det välkomstmeddelande som innehåller information om organisationens namn som du har beviljats åtkomst till.
...
1. Sök genom att ange Adobe ID e-postadress, användarnamn, förnamn eller efternamn.

   * Om din åtkomst är korrekt konfigurerad returnerar sökningen din post.

   * Om värdet i kolumnen **[!UICONTROL ADMIN ROLE]** visar `System` är användaren som visas en systemadministratör.
...
1. Ange ett produktprofilnamn, till exempel _B2B-användare_.

1. Klicka på **[!UICONTROL Next]** och sedan på **[!UICONTROL Save]**.

## Lägg till en användargrupp för profilen {#add-user-group}

En användargrupp är en samling användare som har tilldelats en delad uppsättning behörigheter. Du kan lägga till eller ta bort användare i din användargrupp. Gruppbehörigheterna förblir desamma medan användarna i gruppen ändras.

>[!TIP]
>
>När du migrerar från en tidigare distribution och lägger till befintliga användare till Marketo Engage kan du hoppa över skapandet av användargruppen och helt enkelt öppna den befintliga gruppen och lägga till Marketo Engage produktprofil.
...
1. Ange ett namn för användargruppen, till exempel _B2B-användare_, och klicka på **[!UICONTROL Save]**.
...
1. Klicka på **+** och lägg till varje instans av följande produkter:
   ...
   * [!UICONTROL Adobe Experience Platform - Default Production All Access]
   ...
   ![Admin Console - användargrupp - lägg till produkter](./assets/admin-console-user-group-add-products.png){width="550" zoomable="yes"}

1. Klicka på **[!UICONTROL Save]**.

## Lägga till användare i en ny grupp {#add-users}

Mer information om användarhantering finns i [_Adobe Admin Console-användare_](https://helpx.adobe.com/enterprise/using/users.html){target="_blank"} i Admin Console-dokumentationen.
...
1. Lägg till varje användare:
   ...
   * Ange användarens e-postadress, förnamn och efternamn.
     ...
     ![Experience Platform - lägg till användarprofil för den nya rollen](./assets/admin-console-add-users.png){width="600" zoomable="yes"}
...
## Redigera roller för produktbehörigheter {#edit-roles-for-product-permissions}

Behörigheter är enhetsbehörigheter som gör att du kan definiera de behörigheter som tilldelats en produktprofil. Varje behörighet grupperas under en funktion, till exempel _resor_ eller _inköpsgrupper_. Dessa funktioner representerar funktioner eller objekt i Journey Optimizer B2B Edition.
...
### B2B-produktbehörigheter {#b2b-product-permissions}
...
| B2B-resor | Konfigurera, hantera, visa och publicera behörigheter för B2B-resor. Dessa behörigheter omfattar konto- och personåtgärder, händelseavlyssnare och delade sökvägar. | <li>Hantera B2B-resor |
| Journey Optimizer Rules | Få tillgång till och konfigurera frekvensregler (kommunikationsgränser). Dessa behörigheter bör begränsas till produktadministratörer. | <li>Visa frekvensregler <li>Hantera frekvensregler |
...
| B2B-säljare | <li>Visa kontrollpanelen för B2B-engagemang <li>Få åtkomst till insikter i CRM |
...
I följande exempel vill du lägga till behörigheter som är relaterade till B2B-resursen för användare som har tilldelats rollen B2B-kanalhanterare. Den här ändringen gör att användare i den rollen även kan hantera kontoresor.

>[!NOTE]
>
>En Admin Console-systemadministratör kan utföra dessa steg.

_Ändra behörigheter för en roll :_
...
1. Klicka på **[!UICONTROL Add Groups]**.
   ...
   ![Experience Platform - lägg till grupper i rollen](./assets/aep-permissions-role-add-groups.png){width="700" zoomable="yes"}
...
1. Klicka på **[!UICONTROL Save]** när du har markerat alla grupper som du vill lägga till.
...
1. Lägg till produktbehörigheter för B2B:
   ...
   Se listan över [B2B-produktbehörigheter](#b2b-product-permissions) för att ta reda på vilka produktfunktioner du vill ha för rollen.
